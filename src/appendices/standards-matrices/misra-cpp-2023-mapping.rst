...
.. list-table::
   ...
   * - Rule 8.2.9
     -
     - Rust has type\_id and type\_name functions that are always evaluated at compile time, even if they maybe take an argument (std::any::{type\_name, type\_name\_of\_val, TypeId::of}). Because they are never evaluated at runtime this rule does not apply. Rust also has a type\_if function that is evaluated at runtime (std::any::Any::type\_id). Because it just takes a self reference there and is always evaluated at runtime there is no confusion about potential sideeffects happening at runtime. This function also does not panic.
   * - Rule 8.3.1
     -
